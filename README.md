...
As an optional second parameter a region defined by `x`, `y`, `width` and `height` can be specified to which the search for a QR code should be limited. By default, the region spans the whole image.
